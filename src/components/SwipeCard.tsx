"use client";

import {
  motion,
  useMotionValue,
  useTransform,
  useAnimationControls,
  type PanInfo,
} from "framer-motion";
import Image from "next/image";
import { useCallback, forwardRef, useImperativeHandle } from "react";

export interface SwipeCardHandle {
  animateOut: (direction: "left" | "right") => Promise<void>;
}

interface SwipeCardProps {
  imageUrl: string;
  designId: string;
  onSwipe: (designId: string, liked: boolean) => void;
  isTop: boolean;
  stackIndex: number;
}

const SWIPE_THRESHOLD = 80;
const VELOCITY_THRESHOLD = 500;
const FLY_DISTANCE = 600;

const SwipeCard = forwardRef<SwipeCardHandle, SwipeCardProps>(
  function SwipeCard({ imageUrl, designId, onSwipe, isTop, stackIndex }, ref) {
    const controls = useAnimationControls();
    const x = useMotionValue(0);

    const rotate = useTransform(x, [-FLY_DISTANCE, 0, FLY_DISTANCE], [-12, 0, 12]);
    const likeOpacity = useTransform(x, [0, SWIPE_THRESHOLD], [0, 1]);
    const dislikeOpacity = useTransform(x, [-SWIPE_THRESHOLD, 0], [1, 0]);

    const stackScale = isTop ? 1 : 1 - stackIndex * 0.03;
    const stackY = isTop ? 0 : stackIndex * 6;

    const animateOut = useCallback(
      async (direction: "left" | "right") => {
        const targetX = direction === "right" ? FLY_DISTANCE : -FLY_DISTANCE;
        await controls.start({
          x: targetX,
          rotate: direction === "right" ? 12 : -12,
          opacity: 0,
          transition: {
            type: "spring",
            stiffness: 600,
            damping: 40,
            opacity: { duration: 0.2 },
          },
        });
        onSwipe(designId, direction === "right");
      },
      [controls, designId, onSwipe]
    );

    useImperativeHandle(ref, () => ({ animateOut }), [animateOut]);

    const handleDragEnd = useCallback(
      (_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
        const { offset, velocity } = info;

        const swipedRight =
          offset.x > SWIPE_THRESHOLD || velocity.x > VELOCITY_THRESHOLD;
        const swipedLeft =
          offset.x < -SWIPE_THRESHOLD || velocity.x < -VELOCITY_THRESHOLD;

        if (swipedRight || swipedLeft) {
          const direction = swipedRight ? 1 : -1;
          controls.start({
            x: direction * FLY_DISTANCE,
            rotate: direction * 15,
            opacity: 0,
            transition: {
              type: "spring",
              stiffness: 500,
              damping: 35,
              velocity: velocity.x,
              opacity: { duration: 0.2, delay: 0.05 },
            },
          });
          setTimeout(() => onSwipe(designId, swipedRight), 150);
        } else {
          controls.start({
            x: 0,
            rotate: 0,
            transition: {
              type: "spring",
              stiffness: 500,
              damping: 30,
              mass: 0.8,
            },
          });
        }
      },
      [controls, designId, onSwipe]
    );

    return (
      <motion.div
        className="absolute inset-0"
        style={{
          zIndex: 10 - stackIndex,
          pointerEvents: isTop ? "auto" : "none",
        }}
        initial={{ scale: 0.95, opacity: 0, y: 16 }}
        animate={{
          scale: stackScale,
          opacity: stackIndex > 2 ? 0 : 1,
          y: stackY,
          transition: {
            type: "spring",
            stiffness: 400,
            damping: 30,
            opacity: { duration: 0.25 },
          },
        }}
        exit={{ opacity: 0, transition: { duration: 0.12 } }}
      >
        <motion.div
          className="h-full w-full cursor-grab active:cursor-grabbing"
          style={{ x, rotate }}
          drag={isTop ? "x" : false}
          dragConstraints={{ left: 0, right: 0 }}
          dragElastic={0.7}
          onDragEnd={handleDragEnd}
          animate={controls}
        >
          <div className="relative h-full w-full overflow-hidden rounded-lg bg-neutral-100 shadow-sm ring-1 ring-black/[0.04]">
            <Image
              src={imageUrl}
              alt={`Design ${designId}`}
              fill
              className="object-cover"
              priority={isTop}
              sizes="(max-width: 640px) 100vw, 480px"
            />

            {/* Like indicator — minimal */}
            <motion.div
              className="absolute left-4 top-4"
              style={{ opacity: likeOpacity }}
            >
              <div className="rounded-full bg-white/90 px-3 py-1 shadow-sm backdrop-blur-sm">
                <span className="text-xs font-medium text-neutral-900">Yes</span>
              </div>
            </motion.div>

            {/* Dislike indicator — minimal */}
            <motion.div
              className="absolute right-4 top-4"
              style={{ opacity: dislikeOpacity }}
            >
              <div className="rounded-full bg-white/90 px-3 py-1 shadow-sm backdrop-blur-sm">
                <span className="text-xs font-medium text-neutral-900">No</span>
              </div>
            </motion.div>
          </div>
        </motion.div>
      </motion.div>
    );
  }
);

export default SwipeCard;
